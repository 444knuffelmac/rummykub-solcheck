...
def finalcalculatorrecursive(piececolnumb, memory):
    ...
    possible, possiblerows = possiblewithrows(uniquenumbers, jokers)
    del jokers,uniquenumbers

    if not possible:
        del possiblerows,possible
        memory[str(piececolnumb)] = [False, []]
        del piececolnumb
        return False, []
...
with shelve.open('e') as memoryd:
    ...
    print(end - start)
    memoryd.clear()
    print(finalcalculatorrecursive(piecescolornumber,memoryd))
