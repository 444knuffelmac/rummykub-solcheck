...
def finalcalculatorrecursive(piececolnumb):
    if not piececolnumb:
        return True, []
    piecescolnumbtuple = tuple(piececolnumb)
    if piecescolnumbtuple in memory:
        m =  memory.get(piecescolnumbtuple)
        return m[0], m[1]
    ...
    if jokers == len(piececolnumb):
        ...
        while 0 < i:
            ...
            i -= 1
        memory[piecescolnumbtuple] = [True, returnvalue]
    ...
    if not stillpos:
        memory[piecescolnumbtuple] = [False, []]
        return False, []
