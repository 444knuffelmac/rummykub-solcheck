...
def sortstringly(pscolornumber:list):
    pscolorstring,pscolornumber2 = [],[]
    for x in range(len(pscolornumber)): pscolorstring.append(str(pscolornumber[x])[::-1])
    pscolorstring = sorted(pscolorstring)
    for x in range(len(pscolorstring)): pscolornumber2.append(int(pscolorstring[x]))
...
def firstfilter(noneighbours, jokercounted):
    return False if (jokercounted and (len(noneighbours) > 0)) else True
